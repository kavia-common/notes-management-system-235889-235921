...
# Enhanced to apply app schema on every startup in a repeatable way.
DB_NAME="myapp"
DB_USER="appuser"
...
if [ "${POSTGRES_ALREADY_RUNNING}" = "false" ] && pgrep -f "postgres.*-p ${DB_PORT}" > /dev/null 2>&1; then
...
# Wait for PostgreSQL to start/be ready
...
# Set up user and permissions with proper schema ownership (idempotent)
...
# Save connection command to a file (authoritative source for other scripts)
echo "psql postgresql://${DB_USER}:${DB_PASSWORD}@localhost:${DB_PORT}/${DB_NAME}" > db_connection.txt
echo "Connection string saved to db_connection.txt"

# Save environment variables to a file (for local DB visualizer convenience)
...
# Apply application schema (repeatable, safe to run every time)
echo "Applying application schema..."
bash "$(dirname "$0")/provision_schema.sh"
